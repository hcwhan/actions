...
async function abortChild(child) {
    ...
    try {
        for (let attempt = 1; attempt <= MAX_ABORT_ATTEMPTS; attempt++) {
            ...
            core.info(`Watchdog: 第 ${attempt}/${MAX_ABORT_ATTEMPTS} 次优雅中止尝试，发送 SIGINT（进程树 root pid=${rootPid}）`);
        ...
        core.info(`Watchdog: ${MAX_ABORT_ATTEMPTS} 次优雅中止已耗尽，强制终止进程树 pid=${rootPid}`);
        if (!forceKillProcessTree(rootPid)) {
            core.warning(`Watchdog: 强制终止可能失败，进程树可能仍在运行（root pid=${rootPid}）`);
        }
        await waitForProcessTreeEmpty(rootPid, ABORT_RETRY_INTERVAL_MS);
        if (!isProcessTreeEmpty(rootPid)) {
            core.warning(`Watchdog: 强制终止后进程树仍有残留（root pid=${rootPid}）`);
        }
...
export function createWatchdog(child, jobStartMs, limitMs) {
    ...
    const deadlineTimer = setTimeout(() => {
        ...
        core.info(`Watchdog: 任务已运行 ${elapsedHours.toFixed(1)}小时 >= ${limitHours.toFixed(1)}小时，开始优雅中止`);
        aborted = true;
