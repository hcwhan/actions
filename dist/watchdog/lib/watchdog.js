...
async function abortChild(child) {
    try {
        for (let attempt = 1; attempt <= MAX_ABORT_ATTEMPTS; attempt++) {
            ...
            core.info(`Watchdog: 第 ${attempt}/${MAX_ABORT_ATTEMPTS} 次优雅中止尝试，发送 SIGINT（child.kill）`);
            if (!sendChildSigint(child)) {
                core.warning(`Watchdog: 第 ${attempt}/${MAX_ABORT_ATTEMPTS} 次 SIGINT 发送失败`);
            }
        ...
        core.info(`Watchdog: ${MAX_ABORT_ATTEMPTS} 次优雅中止已耗尽，强制终止 pid=${child.pid}`);
        if (!forceKillProcessTree(child.pid)) {
            core.warning(`Watchdog: 强制终止可能失败，子进程可能仍在运行（pid=${child.pid}）`);
        }
