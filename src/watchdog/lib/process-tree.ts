...
function getProcessParentMapWindows(): Map<number, number> {
  const script =
...
// Unix：ps -eo pid=,ppid=
function getProcessParentMapUnix(): Map<number, number> {
...
function parseParentMapLines(output: string): Map<number, number> {
...
export function listLiveProcessTreePids(rootPid: number): number[] {
...
// 进程树是否已清空
export function isProcessTreeEmpty(rootPid: number): boolean {
  try {
...
// 等待进程树清空；超时返回 false
export async function waitForProcessTreeEmpty(
  rootPid: number,
  timeoutMs: number,
): Promise<boolean> {
