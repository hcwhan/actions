...
async function paginateActionsCaches(prefix: string, apiTryCount: number, retryLabel: string): Promise<CacheEntry[]> {
    ...
    do: async () => {
      ...
      while (page <= MAX_CACHE_LIST_PAGES) {
        ...
        if (page === MAX_CACHE_LIST_PAGES) {
          throw new Error(
            `cache 列举前缀=${prefix} 超过 ${MAX_CACHE_LIST_PAGES} 页（最多 ${MAX_CACHE_LIST_PAGES * 100} 条）上限`,
          );
        }
...
export async function cacheKeyFullExists(cacheKeyFull: string, options: CacheApiOptions): Promise<boolean> {
...
export async function resolveNewestCacheKey(cacheKey: string, options: CacheApiOptions): Promise<string | null> {
  const { apiTryCount } = options;
...
// 按 cache id 删除单条远端条目
async function deleteCacheById(cacheId: number, apiTryCount: number): Promise<void> {
  const octokit = createOctokit();
...
export async function deleteStaleFamilyKeys(familyKey: string, keepCacheKeyFull: string, options: CacheApiOptions): Promise<number> {
  const { apiTryCount } = options;
