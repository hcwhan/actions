...
// 校验 key 片段长度与字符集
function validateCacheKeyPart(key: string, name: string): void {
  ...
  if (name === "cache-key" && trimmed.length > MAX_CACHE_KEY_BASE_LENGTH) {
    throw new Error(
      `${name} 无效：过长（${trimmed.length} 字符）；加上时间后缀后不得超过 ${MAX_CACHE_KEY_BASE_LENGTH + VERSIONED_TIMESTAMP_SUFFIX_LENGTH} 字符`,
    );
  }
}
